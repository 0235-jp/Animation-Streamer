...
describe('createGenerationRouter', () => {
  ...
  it('returns combined result for non-stream requests', async () => {
    ...
    const response = await request(app)
      .post('/api/generate')
      .send({ presetId: 'anchor-a', stream: false, debug: true, requests: [{ action: 'speak', params: { text: 'hello' } }] })
  ...
  it('maps ActionProcessingError to response status and payload', async () => {
    ...
    const response = await request(app)
      .post('/api/generate')
      .send({ presetId: 'anchor-a', requests: [{ action: 'idle', params: { durationMs: 500 } }] })
  ...
  it('streams NDJSON chunks for stream requests', async () => {
    ...
    const response = await request(app)
      .post('/api/generate')
      .send({ presetId: 'anchor-a', stream: true, debug: true, requests: [{ action: 'idle', params: { durationMs: 300 } }] })
  ...
  it('returns NDJSON error chunk when ActionProcessingError occurs during streaming', async () => {
    ...
    const response = await request(app)
      .post('/api/generate')
      .send({ presetId: 'anchor-a', stream: true, requests: [{ action: 'idle', params: { durationMs: 100 } }] })
  ...
  it('handles unexpected errors with 500 status for non-stream requests', async () => {
    ...
    const response = await request(app)
      .post('/api/generate')
      .send({ presetId: 'anchor-a', requests: [{ action: 'idle', params: { durationMs: 200 } }] })
  ...
  it('allows requests when API key matches', async () => {
    ...
    const response = await request(app)
      ...
      .send({ presetId: 'anchor-a', stream: false, debug: false, requests: [{ action: 'idle', params: { durationMs: 200 } }] })
